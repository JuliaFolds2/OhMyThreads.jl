...
using OhMyThreads: BatchSplit, ScatterSplit
    ...
    for (; ~, f, op, itrs, init) in sets_to_test
        @testset "f=$f, op=$op, itrs::$(typeof(itrs))" begin
            @testset for sched in (
                ...
                @testset for split in (BatchSplit(), ScatterSplit())
...
@testset "ChunkSplitters.Chunk" begin
    x = rand(100)
    chnks = OhMyThreads.chunk_indices(x; n = Threads.nthreads())
    for scheduler in (
    ...
    # enumerate(chunks)
    data = 1:100
    @test tmapreduce(+, enumerate(OhMyThreads.chunk_indices(data; n=5)); chunking=false) do (i, idcs)
        [i, sum(@view(data[idcs]))]
    end == [sum(1:5), sum(data)]
    @test tmapreduce(+, enumerate(OhMyThreads.chunk_indices(data; size=5)); chunking=false) do (i, idcs)
        [i, sum(@view(data[idcs]))]
    end == [sum(1:20), sum(data)]
    @test tmap(enumerate(OhMyThreads.chunk_indices(data; n=5)); chunking=false) do (i, idcs)
        [i, idcs]
...
@testset "macro API" begin
    ...
    # enumerate(chunks)
    data = collect(1:100)
    @test @tasks(for (i, idcs) in enumerate(OhMyThreads.chunk_indices(data; n=5))
        @set reducer = +
        @set chunking = false
        [i, sum(@view(data[idcs]))]
    end) == [sum(1:5), sum(data)]
    @test @tasks(for (i, idcs) in enumerate(OhMyThreads.chunk_indices(data; size=5))
        @set reducer = +
        [i, sum(@view(data[idcs]))]
    end) == [sum(1:20), sum(data)]
    @test @tasks(for (i, idcs) in enumerate(OhMyThreads.chunk_indices(1:100; n=5))
        @set chunking=false
        @set collect=true
        [i, idcs]
    end) == [[1, 1:20], [2, 21:40], [3, 41:60], [4, 61:80], [5, 81:100]]
...
@testset "top-level kwargs" begin
    ...
    @test tmapreduce(sin, +, 1:10000; split = ScatterSplit()) ≈ res_tmr
    ...
    @test_throws ArgumentError tmapreduce(
        sin, +, 1:10000; split = ScatterSplit(), scheduler = StaticScheduler())
    @test_throws ArgumentError tmapreduce(
