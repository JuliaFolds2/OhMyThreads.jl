...
- ![Feature][badge-feature] Within a `@tasks` block one can now mark a region as with `@one_only`. This region will be run by a single parallel task only (other tasks will skip over it).
